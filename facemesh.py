...
class BaseInferencer:
    ...
    def __init__(self, model_path, edgetpu=True):
        experimental_delegates = [tflite.load_delegate(EDGETPU_SHARED_LIB)] if edgetpu else None
...
class Classifier():
    ...
    def __init__(self, model_path, labels_path, edgetpu=True):
        experimental_delegates = [tflite.load_delegate(EDGETPU_SHARED_LIB)] if edgetpu else None
        self.interpreter = tflite.Interpreter(
            model_path=model_path,
            experimental_delegates=
    ...
    def inference(self, image):
        ...
        cv2_im_rgb = cv2.resize(image, self.inference_size)
        run_inference(self.interpreter, cv2_im_rgb.tobytes())
        classes = get_classes(self.interpreter, 1, 0)

        prediction = None
        score = -1

        for kclass in classes:
            if kclass.score > score:
                prediction = kclass.id
                score = kclass.score
